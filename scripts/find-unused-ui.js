...
const uiFiles = fs.readdirSync(uiDir).filter(file => file.endsWith('.tsx') || file.endsWith('.ts'));
...
for (const uiFile of uiFiles) {
  ...
  for (const sourceFile of sourceFiles) {
    ...
    try {
      ...
      const importPattern = new RegExp(`from\\s+["']@/components/ui/${componentName}["']|from\\s+["']\\./ui/${componentName}["']`, 'g');
